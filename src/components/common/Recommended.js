...
class Recommended extends Component {
	...
	render () {
		const list = this.state.news.map((item, index) => {
			return this.renderNews(index, item);
		});
		const indicators = this.state.news.map((elem, index) => {
			return this.renderIcons(elem, index);
		});
		...
		return (
			<View>
				...
				<Animated.View
					style={{flexDirection: 'row', left: this.containerPosition}}
					{...this.state.panResponder.panHandlers}
				>{list}</Animated.View>
				<View style={indicatorsContainer}>{indicators}</View>
			</View>
		);
	}
}
